...
use crate::runtime::{
	init,
	jsapi_utils::{exception::report_and_clear_exception, print::println_value},
	modules,
};
...
pub(crate) fn eval_module(path: &Path) {
	...
	rooted!(in(rt.cx()) let module = modules::compile_module(rt.cx(), &String::from(path.file_name().unwrap().to_str().unwrap()), &script));
