...
pub(crate) fn impl_wrapper_fn(mut function: ItemFn, class_ty: Option<&Type>, keep_inner: bool, is_constructor: bool) -> Result<(ItemFn, ItemFn, Parameters)> {
	let krate = quote!(::ion);
	...
	let wrapper_args: Vec<FnArg> = if is_constructor {
		vec![parse_quote!(cx: &'cx #krate::Context), parse_quote!(args: &'a mut #krate::Arguments<'cx>), parse_quote!(this: &mut #krate::Object<'cx>)]
	} else {
